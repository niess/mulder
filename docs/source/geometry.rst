...
location function, :math:`L`, is sufficient for this purpose, it is usually not
...
.. math::
   ...
   \left|
     \begin{array}{rcl}
       ...
       (\vec{r}, \vec{u}) & \longmapsto & (\lambda, i) \\
     \end{array}
   \right. ,

where the returned :math:`\lambda` value is the distance to the first (closest
from :math:`\vec{r}`) intersected medium boundary, and :math:`i` the index of
